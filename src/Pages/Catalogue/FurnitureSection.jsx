...
const FurnitureSection = ({ handleSubmit, initialize }) => {
  useEffect(() => {
    const savedData = localStorage.getItem("furnitureSectionForm");
    if (savedData) {
  ...
  const onSubmit = (formValues) => {
    let section = "furniture-section";
    CatalogueData(formValues, section); // Assuming this is your action to send data to the backend
    localStorage.setItem("furnitureSectionForm", JSON.stringify(formValues)); // Save to localStorage
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <h2 className="text-center mt-3">Furniture Section</h2>
      <div className="m-3">
