...
const ApplianceSectionIndex = () => {
  ...
  const fetchCatalogues = async () => {
    try {
      const response = await fetch(`${apiUrl}/api/appliance-section`);
  ...
  const deleteCatalogue = async (id) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/appliance-section/${id}`, {
        method: "DELETE",
      });
      const data = await response.json();
  ...
  return (
    <div className="container mt-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>Appliance Sections</h2>
        <Link to="/appliance-section/create" className="btn btn-primary">
          Create New
        </Link>
      </div>
            ...
            <tbody>
              {catalogues.map((catalogue, index) => (
                <tr key={catalogue._id}>
                  ...
                  <td>
                    <Link
                      to={`/appliance-section/${catalogue._id}`}
...
export default ApplianceSectionIndex;
