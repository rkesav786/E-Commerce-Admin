...
const GrocerySection = ({ handleSubmit, initialize }) => {
  useEffect(() => {
    const savedData = localStorage.getItem("grocerySectionForm");
    if (savedData) {
  ...
  const onSubmit = (formValues) => {
    let section = "grocery-section";
    CatalogueData(formValues, section); // Assuming this is your action to send data to the backend
    localStorage.setItem("grocerySectionForm", JSON.stringify(formValues)); // Save to localStorage
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <h2 className="text-center mt-3">Grocery Section</h2>
      <div className="m-3">
